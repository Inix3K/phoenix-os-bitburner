...
import { queueFactory } from "./lib.utils.so";
...
export const joinFactions = async (ns, player, factions, augmentations) => {
    if (!invited.includes(this.name)) { return; } 
    let blockers = faction_blockers.get(faction_name);
    if (!blockers.length) {
        ns.joinFaction(faction_name);
    } else {
       ns.joinFaction(faction_name); // todo
    }
};
 ...
 */
export const selectFocusActivity = async (ns, player, factions) => {
 

    if (player.work.isWorking) {
        let current = player.work.current.factionName;
        for (let faction of factions) {
            if (faction.name == current) {
            } else {
                // if (faction.augmentations_available)
            }
            
        
            // faction.wanted_augs = augs_focus.filter(aug => aug.factions_offering.includes(faction.name));
        ...
        while (next_priority && !next_faction) {
            ...
            //     faction.wanted_augs = augs_focus.filter(aug => aug.factions_offering.includes(faction.name) && aug.rep > faction.rep);
 ...
 * @param {import("./phoenix-doc").PlayerObject} player 
 */
// export const buyBestAug = async (ns, player) => {
    ...
    // const pq = await prioritize_augmentations(ns, player); // priority queue defines augmentation priority.
    // let next_priority = pq.poll();
    // let focus = await getFocusStat(ns, player);
