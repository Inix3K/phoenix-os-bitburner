 ...
 */
 ...
 /**
  * This now runs the distributed HWGW workload, but includes a share component
  */
 export default class Balanced extends Default {
     ...
     prepare_bulk(attackers, player){
         ...
         for (let a of attackers) {
             if (a.ram.free > 4 && globalThis.ns.ps(a.id).every(proc => proc.filename != "bin.share.loop.js")) {
                bundles.push(JSON.stringify({
                    file: "bin.share.loop.js",
                    attacker: a.id,
                    threads: Math.max(1, a.threadCount(64, false)),
                    args: []
                }));
             }
         }
     ...
     filter_targets() {
         ...
         priorities.set(14, (t => t.level < 200));
